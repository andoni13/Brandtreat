...
class Sidebar extends Component {
	constructor(props) {
		super(props);
		this.state = {
			// the detected height of the sidebar title in pixels
	...
	saveSidebarTitleHeight() {
		const height = ReactDOM.findDOMNode(this.refs.title).offsetHeight;

		if (height !== this.state.sidebarTitleHeight) {
			this.setState({sidebarTitleHeight: height});
		}
	}
	...
	render() {
		const sidebarTitleStyle = {...defaultStyles.sidebarTitle, ...this.props.styles.sidebarTitle};
		...
		if (this.props.sidebarTitle) {
			sidebarStyle.top = this.state.sidebarTitleHeight;
			contentStyle.top = this.state.sidebarTitleHeight;
		}
		...
		if (isTouching) {
			...
			// TODO: TOGGLE SIDEBAR ON DOCKED STATE
			if (this.props.open) {
			// Open sidebar on docked
				sidebarStyle.transform = `translateX(0%)`;
				sidebarStyle.WebkitTransform = `translateX(0%)`;
		...
		const sidebarTitleHTML = (
			<div ref="title" style={sidebarTitleStyle}>{this.props.sidebarTitle}</div>
		)
		return (
			<div {...rootProps}>
...
Sidebar.propTypes = {
	...
	styles: React.PropTypes.shape({
		...
		sidebarTitle: React.PropTypes.object,
