...
const AppBarNew = React.createClass({
	...
	getStyles() {
		...
		let styles = {
			...
			logo: {
				...
				marginBottom: (themeVariables.height - iconButtonSize) - 2,
				marginRight: 8,
			},
			title: {
				whiteSpace: 'nowrap',
	...
	_onLogoTouchTap(event) {
		if (this.props.onLogoTouchTap) {
			this.props.onLogoTouchTap(event);
		}
	},
	...
	render() {
		...
		let logoElement = <img onTouchTap={this._onLogoTouchTap} style={this.prepareStyles(styles.logo, logoStyle)} src="/img/logo.png" />
		let titleElement;
		...
		if (title) {
			...
			// If a title is not present then clear the logo
			logoElement = ''
		}
